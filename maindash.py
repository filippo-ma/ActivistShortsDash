...
# tweet api auth
auth = tweepy.OAuthHandler(st.secrets["twitter_api_key"], st.secrets["twitter_api_key_secret"])
auth.set_access_token(st.secrets["twitter_access_token"], st.secrets["twitter_secret_token"])
...
if option == 'Activist short sellers':
    ...
    for username in selected_users:
        ...
        tweets = api.user_timeline(username, count=60, include_rts=False)
        ...
        col2.subheader(f"{user.name} stocktwits:")
        st.write(' ')
